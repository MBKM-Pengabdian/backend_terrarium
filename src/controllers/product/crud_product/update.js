    ...
    upload(req, res, async function (err) {
      ...
      if (
        ...
        return res.status(400).send({
          status: 400,
          message:
            "productId, User id, Product Name, Price, and Stock Quantity are required fields.",
        });
      }
      ...
      if (req.file) {
        ...
        let sizeAcceptedInPercent = Math.floor(
          (size - (size - config.IMG_LIMIT_SIZE / 2)) / (size / 100)
        );

        // Ensure that sizeAcceptedInPercent is within the valid range (1 to 100)
        sizeAcceptedInPercent = Math.min(
          100,
          Math.max(1, sizeAcceptedInPercent)
        );
